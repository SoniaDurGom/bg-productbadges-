...
| Limitación                        | Detalle                                                                                                                                   |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| **Dependencia del tema**          | La posición en listados depende de que el tema llame a `displayProductListReviews` (p. ej. tema **Classic**).                             |
...
El módulo se ha construido de forma incremental siguiendo un enfoque MVP evolutivo, evitando un único bloque de desarrollo monolítico.

Las fases principales han sido:

- Estructura inicial del módulo (instalación, ObjectModel, base SQL)
- Sistema de configuración global mediante HelperForm y Configuration
- CRUD de badges en back office
- Sistema de asignación many-to-many producto ↔ badge
- Implementación de frontend mediante hooks
- Soporte multilenguaje con tabla `_lang`
- Ajustes de UI en back office y traducciones del módulo
- Soporte de multitienda a nivel de configuración
- Mejora de UX en listados y ficha de producto
- Optimización progresiva de CSS y comportamiento de layout

Este enfoque permite trazar claramente la evolución del módulo y justificar decisiones técnicas tomadas en cada fase.

---

## Autor

Sonia D.s— módulo `productbadges` para prueba técnica PrestaShop 1.7.8.x.